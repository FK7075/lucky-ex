...
public class LSON {
	...
	public LSON(Object jsonObject) {
		...
		if (List.class.isAssignableFrom(clzz)) {
			try {
				jsonStr = listToJsonStr((List<?>) jsonObject);
	 ...
	 */
	private <T> String listToJsonStr(List<T> list) throws IllegalArgumentException, IllegalAccessException {
	 ...
	 * @throws IllegalArgumentException
	 */
	private <T> String fieldToJsonStr(Object field_Obj, Field field)
		...
		if (obj != null) {
			...
			if (clzz.getClassLoader() == null && !clzz.isAssignableFrom(List.class) && !clzz.isArray()) {
				...
				fieldJsonStr += "\"" + field.getName() + "\"" + ":" + listToJsonStr((List<T>) obj);
