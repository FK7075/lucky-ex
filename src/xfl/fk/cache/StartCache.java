...
public class StartCache {
	...
	public <T> List<?> getListCache(T t) {
		SqlInfo f = classUtils.getSqlInfo(ClassInfoFactory.createClassInfo(t), "select");
	...
	public <T> List<?> getPagListCache(T t, int index, int size) {
		SqlInfo f = classUtils.getSqlInfo(ClassInfoFactory.createClassInfo(t), index, size);
		if (lucy.get(createSql.getSqlString(f)) == null) {
			list = autopackage.getTable(t.getClass(), f.getSql(), f.getObj());
			lucy.add(createSql.getSqlString(f), list);
		} else {
	...
	public <T> List<?> getSortListCache(T t, String property, int r) {
		SqlInfo f = classUtils.getSqlInfo(ClassInfoFactory.createClassInfo(t), property, r);
		if (lucy.get(createSql.getSqlString(f)) == null) {
			list = autopackage.getTable(t.getClass(), f.getSql(), f.getObj());
			lucy.add(createSql.getSqlString(f), list);
		} else {
	...
	public <T> boolean saveCache(T t) {
		SqlInfo f = classUtils.getSqlInfo(ClassInfoFactory.createClassInfo(t), "insert");
	...
	public <T> boolean deleteCache(T t) {
		SqlInfo f = classUtils.getSqlInfo(ClassInfoFactory.createClassInfo(t), "delete");
		sqlOperation.setSql(f.getSql(), f.getObj());
		lucy.evenChange();
		return true;
	}
	...
	public <T> boolean updateCache(T t) {
		SqlInfo f = classUtils.getSqlInfo(ClassInfoFactory.createClassInfo(t), "update");
		sqlOperation.setSql(f.getSql(), f.getObj());
		lucy.evenChange();
		return true;
	}
