...
public class LuckyUtils {
	...
	public static void pojoSetId(Object pojo) {
		...
		if(lucy.auto()) {
			...
			ResultSet resultSet = getResultSet(sql,getTableName(clzz),getDatabaseName());
			try {
				while(resultSet.next()) {
					String nextID= resultSet.getString("nextID");
					next_id=Integer.parseInt(nextID);
				}
				Field field=getIdField(clzz);
	...
	//返回实体类对应的ID属性
	public static Field getIdField(Class<?> clzz) {
		Lucky lucy=clzz.getAnnotation(Lucky.class);
		String id=lucy.id();
		Field id_field=null;
